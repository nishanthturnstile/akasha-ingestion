...
class InMemoryFieldQueryRepository:
    ...
    ) -> FieldQueryRecord | None:
        matches = [
            record
            for record in self._queries.values()
            if record.selected_scene_id == selected_scene_id
            and record.geometry_hash == geometry_hash
            and record.index_name == index_name
            and record.analysis_version == analysis_version
            and (record.expires_at is None or record.expires_at > datetime.now(UTC))
        ]
        return max(
            matches,
            key=lambda value: value.created_at or datetime.min.replace(tzinfo=UTC),
            default=None,
        )

    def delete_expired(self, *, limit: int = 100) -> int:
        expired = [
            key
            for key, record in self._queries.items()
        ...
        with self._engine.connect() as connection:
            row = connection.execute(
                text(
                    """
                    SELECT *, ST_AsGeoJSON(field_geometry) AS field_geometry_geojson
                    FROM akasha.field_queries
                    WHERE selected_scene_id = CAST(:selected_scene_id AS uuid)
    ...
    def delete_expired(self, *, limit: int = 100) -> int:
        with self._engine.begin() as connection:
            result = connection.execute(
                text(
                    """
                    DELETE FROM akasha.field_queries
                    WHERE id IN (
                        SELECT id FROM akasha.field_queries
